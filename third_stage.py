...
x_turn, y_turn = changing_system_of_coordinates(x_moon, y_moon, x_ship, y_ship)
r_orbit = sqrt (x_turn**2 + y_turn**2) * 1000
...
def speed_and_coordinate(x, y, v_x, v_y, a_x, a_y, t = int_period):
    x = x + v_x * t + a_x * (t ** 2) / 2
    y = y + v_y * t + a_y * (t ** 2) / 2
    v_x = v_x + a_x * t
    v_y = v_y + a_y * t
...
def free_flight (x, y, v_x, v_y, t = int_period, coordinate = 0):
    while x <= coordinate:
...
def speed_after_impulse (x = 0, y = r_orbit, v_x = - orbital_speed, v_y = 0, rad = Radius):
    height_finish = y
...
def defining_coordinate_of_giving_impulse (orb_speed = orbital_speed):
...
def position_on_the_orbit (x, y, radius):
    if x <= 0:
        alpha = acos( y / radius )
    else:
...
def speed_and_coordinate_after_impulse (x_ship_orbit, y_ship_orbit, orb_speed = orbital_speed, rad = r_orbit, t = int_period, m_ship = mass_space_ship, m_engine = mass_engine, u = u_fuel):
    ...
    while x > 0:
        ...
        result.write(str (x) + '\t \t' + str(y) + '\t \t' + str( sqrt (x**2 + y**2 ) )+ '\t \t'+ str( v_x ** 2 + v_y**2 ) + '\t \t' + str (t_new) + '\n')
    result.close()
...
def writing_coordinates (x, y, v_x, v_y, mass_engine, t_new, f_max = F_crit, t = int_period):
    n=1
    result = open('output landing moon.txt', 'a')
    while y > 0:   # Половина траектории
        ...
        if n % 10 == 0:
            ...
            #для записи в файл
    a_x, a_y = a_x_and_a_y(x, y)
    while (v_x ** 2) < abs ( 2 * x * ( - (f_max / (mass_space_ship + mass_engine)) + a_x )):   # рассчитывается скорость для которой погасится горизонтальная скорость
        a_x, a_y = a_x_and_a_y(x, y)
        x, y, v_x, v_y = speed_and_coordinate(x, y, v_x, v_y, a_x, a_y)
        t_new += t
        t_new = round(t_new * 100) / 100
...
def making_stop (x, y, v_x, v_y, m_engine, t_new, f_max = F_crit, m_ship = mass_space_ship, u = u_fuel, t = int_period):
...
def total_stop (x, y, v_x, v_y, m_engine, t_new, u = u_fuel, m_ship = mass_space_ship, rad = Radius, t = int_period):
    result = open('output landing moon.txt', 'a')
    while v_y > 0:
        a_x_grav, a_y_grav = a_x_and_a_y(x, y)
